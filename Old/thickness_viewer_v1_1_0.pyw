...
class PLCReader:
    ...
    def read_db_raw(self, db_num, total, chunk=400):
        data=bytearray(total); off=0
        while off<total:
            ...
            try:
                ...
                raise RuntimeError(
                    f"DB{db_num} @{off}: {e}\n"
                    "Verifica: S7_Optimized_Access=FALSE  e  PUT/GET abilitato")
            off+=sz
        return data
...
class ThicknessApp(tk.Tk):
    ...
    def _plc_read(self):
        ...
        try:
            ...
            self._plog(f"→ DB{db} ({self._db_size}b)...\n","info")
            t0=time.time()
            raw=self._plc_client.read_db_raw(db,self._db_size)
            self._plog(f"✓ {self._db_size}b in {(time.time()-t0)*1000:.0f}ms\n","ok")
            ...
            sr=sc.get('I_ParametriCntrolloSpessore.SpessoreDiscoRiferimento',
                       sc.get('SpessoreDiscoRiferimento',2.98))
            self._plog(
            ...
            if sc.get('AppSpessoreNok'):
                self._plog(f"  ⚠ DOPPIO SPESSORE — ΔMax>{sc.get('I_ParametriCntrolloSpessore.SpessoreMassimo',1.0):.2f}mm\n","err")
    ...
    def _ae_poll(self):
        ...
        try:
            db=int(self._pv_db.get())
            raw=self._ae_client.read_db_raw(db,self._db_size,chunk=900)
