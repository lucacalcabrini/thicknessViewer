...
class ThicknessApp(tk.Tk):
    ...
    def _draw_profilo(self):
        ...
        if self.db_data is None:
            ax.text(0.5,0.5,"Nessun dato — leggi dal PLC (PLC Reader → Carica Viewer)",
        ...
        # ── Baseline (quota supporto) — opzionale, retrocessa ─────────────
        bas = ar.get('aBaseline',[])
        if len(bas)>=n and self._ck_base.get():
            ba=np.array(bas[:n]); mask=ba>1e-6
            if mask.any():
                ax.plot(x[mask],ba[mask],color=BASELINE_CLR,lw=0.8,ls=':',
                        alpha=0.5,label='Baseline (quota supporto) [mm]')

        # ── Profilo spessore assoluto — linea principale ───────────────────
        prof = ar.get('aProfiloSpessore',[])
        prof_arr = None
        if len(prof)>=n and self._ck_prof.get():
            prof_arr=np.array(prof[:n])
            # Celle campionate (esclude celle mai viste: valore vicino a 0 senza baseline)
            nraw = ar.get('aNraw',[])
            mask_camp = np.ones(n,dtype=bool)
            if len(nraw)>=n:
                mask_camp = np.array(nraw[:n]) > 0
            if mask_camp.any():
                ax.plot(x[mask_camp],prof_arr[mask_camp],
                        color=PROFILE_CLR,lw=2.0,
                        label=f'Spessore misurato [mm]',zorder=5)

        # ── Bande di tolleranza e spessore atteso ─────────────────────────
        if self._ck_thresh.get():
            # Linea spessore atteso (target)
            ax.axhline(sp_att, color=OK_CLR, lw=1.5, ls='-', alpha=0.8,
                       label=f'Spessore atteso: {sp_att:.2f} mm')
            # Banda OK: spessore_atteso ± 0.5*SpessoreMassimo (visualizzazione, non il PLC)
            ax.axhline(sp_att + sg, color=THRESHOLD_CLR, lw=1.2, ls='--',
                       alpha=0.9, label=f'Soglia NOK: {sp_att+sg:.2f} mm (+{sg:.2f})')
            # Fill zona OK
            ax.axhspan(max(0, sp_att - sg*0.3), sp_att + sg,
                       alpha=0.06, color=OK_CLR, zorder=1)
            # Fill zona NOK (sopra soglia)
            ax.axhspan(sp_att + sg, sp_att + sg*3,
                       alpha=0.06, color=ERR_CLR, zorder=1)

        # ── Celle NOK evidenziate ─────────────────────────────────────────
        if self._ck_nok.get():
            dlt = ar.get('aProfiloDelta',[])
            nraw = ar.get('aNraw',[])
            if len(dlt)>=n and len(nraw)>=n and prof_arr is not None:
                da=np.array(dlt[:n])
                mask_camp = np.array(nraw[:n]) > 0
                mask_nok  = mask_camp & (da > sg)
                if mask_nok.any():
                    ax.scatter(x[mask_nok], prof_arr[mask_nok],
                               color=FUORI_CLR, s=35, marker='x', lw=2.0,
                               label=f'Celle NOK: Δ>{sg:.2f}mm ({int(mask_nok.sum())})',
                               zorder=9)
                    # Evidenzia anche l'area sotto con fill rosso
                    for xi, yi in zip(x[mask_nok], prof_arr[mask_nok]):
                        ax.vlines(xi, sp_att + sg, yi,
                                  colors=FUORI_CLR, lw=0.8, alpha=0.4)

        ax.set_xlabel("Posizione asse [mm]", color=TEXT_CLR, fontsize=10)
        ax.set_ylabel("Spessore misurato [mm]", color=TEXT_CLR, fontsize=10)
        ...
        ttl = (f"{fn}   {esito}   celle fuori: {nf}/{ns}   dir: {dls}"
               if ts else fn)
        ax.set_title(ttl, color=ERR_CLR if nok_flag else OK_CLR, fontsize=9, pad=6)

        # Linee bordo zona di misura
    ...
    def _draw_delta(self):
        ...
        if len(dlt)>=n:
            ...
            if len(nraw)>=n:
                mask_camp = np.array(nraw[:n]) > 0
            mask = mask_camp & (np.abs(da) > 1e-6)

            if mask.any():
                xm=x[mask]; dm=da[mask]
                ax.fill_between(xm,0,dm,where=(dm>=0)&(dm<=sg),
                                color=OK_CLR,alpha=0.30,
                                label='Eccesso entro tolleranza (OK)')
                ax.fill_between(xm,sg,dm,where=(dm>sg),
                                color=ERR_CLR,alpha=0.55,
                                label='Eccesso oltre soglia NOK')
                ax.fill_between(xm,dm,0,where=(dm<0),
                                color=ACCENT,alpha=0.20,
                                label='Difetto (disco più sottile)')
                ax.plot(xm,dm,color=DELTA_CLR,lw=1.5,zorder=5)
                mask_nok = mask_camp & (da > sg)
                if mask_nok.any():
                    ax.scatter(x[mask_nok],da[mask_nok],
                               color=FUORI_CLR,s=40,marker='x',lw=2.0,
                               label=f'Celle NOK ({int(mask_nok.sum())})',zorder=9)

        ax.axhline(0,color=OK_CLR,lw=1.5,ls='-',alpha=0.8,
                   label=f'Δ=0  (atteso: {sp_att:.2f}mm)')
        ax.axhline(sg,color=ERR_CLR,lw=1.5,ls='--',alpha=0.9,
                   label=f'Soglia NOK = {sg:.2f}mm')
        ax.axhspan(0,sg,alpha=0.04,color=OK_CLR,zorder=0)
        ax.axhspan(sg,sg*4,alpha=0.04,color=ERR_CLR,zorder=0)

        ax.set_xlabel("Posizione asse [mm]",color=TEXT_CLR,fontsize=10)
        ax.set_ylabel("Δ spessore (misurato − atteso) [mm]",color=TEXT_CLR,fontsize=10)
        ...
        esito = "✗ NOK" if nok_flag else "✓ OK"
        ax.set_title(
            f"Deviazione dal target ({sp_att:.2f}mm)  —  {esito}  — celle fuori: {nf}/{ns}",
