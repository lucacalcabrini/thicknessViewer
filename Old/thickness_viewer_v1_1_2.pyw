...
FB_TARGET   = "Fb936_ControlloSpessore_v11"
FB_SCL_NAME = '"Fb936_ControlloSpessore_v11"'   # nome esatto nel file .db TIA Portal
...
# ── Correzione offset manuale (se la mappa è globalmente shiftata) ──────────
# Mettere qui la differenza rilevata da TIA Portal:
#   DB_OFFSET_CORRECTION = offset_TIA(variabile_nota) - offset_mio(variabile_nota)
# Esempio: se TIA mostra AppSpessoreMedio a byte 4970 e io calcolo 4974,
#   DB_OFFSET_CORRECTION = 4970 - 4974 = -4
# Con 0 la correzione è disabilitata.
DB_OFFSET_CORRECTION = 0
...
class _OB:
    ...
    def skip_rtrig(self): self._a(); self.off+=2
    def skip_ptr(self):   self._a(); self.off+=6


def build_offset_map():
    b = _OB()
    # ── UDT VAR_INPUT RETAIN ─────────────────────────────────────
    ...
    b.bools(['O_SpessoreOk','O_SpessoreNOK','O_AlmControlloDisattivo',
             'O_TaraturaAttiva','O_TaraturaCompletata'])                # 68.x
    b._a()                                                             # → 70
    b.real('O_SpessoreMedio')                                          # 70
    b.real('O_SpessoreMax')                                            # 74
    b.real('O_DeltaMedio')                                             # 78
    b.real('O_DeltaMax')                                               # 82
    b.int16('O_nCelleProfilo')                                         # 86
    b.int16('O_nCelleFuoriSoglia')                                     # 88
    b.bools(['O_LaserInRange'])                                        # 90.0
    b._a()                                                             # → 92
    # ── VAR_IN_OUT ────────────────────────────────────────────────
    b.skip_ptr(); b.skip_ptr()                                        # 92, 98
    # ── VAR Pinza ─────────────────────────────────────────────────
    b.bools(['Pinza.InZonaControllo','Pinza.InZonaRallenta'])          # 104.x
    b._a()
    b.lreal('Pinza.OvrAutoOld')                                        # 106
    b.lreal('Pinza.OvrManOld')                                         # 114
    b.real('Pinza.nLettureConsecutive')                                # 122
    # ── VAR RETAIN ────────────────────────────────────────────────
    b.arr_r('aBaseline', ARRAY_SIZE)                                   # 126
    b.bools(['baselineValida'])                                         # 930.0
    b._a()                                                             # → 932
    # ── R_TRIG / F_TRIG ──────────────────────────────────────────
    for _ in range(5): b.skip_rtrig()                                 # 932..940
    b.bools(['DirOk','Ripeti'])                                        # 942.x
    b._a()
    b.int16('AppMinIndexSurce')                                        # 944
    b.int16('AppMaxIndexSurce')                                        # 946
    b.int16('AppMaxIndex')                                             # 948
    b.bools(['taraturaInCorso'])                                       # 950.0
    b._a()                                                             # → 952
    # ── Buffer profilometro ───────────────────────────────────────
    b.arr_r('aSomRaw', ARRAY_SIZE)                                     # 952
    b.arr_i('aNraw',   ARRAY_SIZE)                                     # 1756
    b.arr_r('aSomCal', ARRAY_SIZE)                                     # 2158
    b.arr_i('aNcal',   ARRAY_SIZE)                                     # 2962
    b.arr_r('aProfiloSpessore', ARRAY_SIZE)                            # 3364
    b.arr_r('aProfiloDelta',    ARRAY_SIZE)                            # 4168
    # ── Scalari risultati ─────────────────────────────────────────
    b.bools(['AppSpessoreOk','AppSpessoreNok'])                        # 4972.x
    b._a()
    b.real('AppSpessoreMedio')                                         # 4974
    b.real('AppSpessoreMax')                                           # 4978
    b.real('AppDeltaMedio')                                            # 4982
    b.real('AppDeltaMax')                                              # 4986
    b.int16('AppNcelleProfilo')                                        # 4990
    b.int16('AppNcelleFuoriSoglia')      # ← v1.1: da TEMP a VAR       # 4992
    b.bools(['AppValidValue'])                                         # 4994.0
    b._a()
    b.int16('OldDirLavoro')                                            # 4996
    b.bools(['AuxFpCambioDirezione'])                                  # 4998.0
    b._a()                                                             # → 5000
    if DB_OFFSET_CORRECTION != 0:
        corrected = []
        for name, off, dtype, sz in b.e:
            new_off = off + DB_OFFSET_CORRECTION
            if new_off >= 0:
                corrected.append((name, new_off, dtype, sz))
        return corrected, b.off
    return b.e, b.off
        ...
        while off < limit:
            ...
            try:
                ...
                if 'out of range' in err or 'address' in err:
                    # DB termina qui: zero-fill il resto (già fatto da bytearray)
                    break
                raise RuntimeError(
                    f"DB{db_num} @{off}: {e}\n"
...
class ThicknessApp(tk.Tk):
    ...
    def _build_left(self, P):
        ...
        for k,lbl in [
                ...
                ('SpessoreMassimo','Max delta NOK [mm]'),  # v1.1: è delta, non assoluto
                ('nLettureConsecutiveAllarme','N. celle consec.'),
                ('OvrTrasfertPerpassaggio','Override %'),
                ('SpessoreDiscoRiferimento','Disco rif. [mm]'),
    ...
    def _tab_plc(self, P):
        ...
        self._btn_read=ttk.Button(r2,text="📥 Leggi DB",style="Plc.TButton",
                                   command=self._plc_read,state="disabled")
        self._btn_read.pack(side="left",padx=8)
        self._btn_load=ttk.Button(r2,text="📊 Carica Viewer",
                                   command=self._plc_load,state="disabled")
        ...
        tk.Label(info,
            text=(f"Db16070  •  Dimensione stimata: {self._db_size} byte  "
                  f"(la dimensione reale viene rilevata automaticamente alla lettura)  •  "
    ...
    def _plc_connect(self):
        try:
            ...
            self._btn_read.config(state="normal")
            self._save_ini()
        except Exception as e:
            self._plog(f"✗ {e}\n","err")
    ...
    def _plc_disconnect(self):
        ...
        self._btn_read.config(state="disabled"); self._btn_load.config(state="disabled")
    ...
    def _plc_read(self):
        ...
        try:
            ...
            # ── Diagnostica offset: mostra parametri UDT attesi vs letti ──────
            # Se questi valori NON corrispondono al file .db, l'offset map è shiftata.
            # In quel caso aprire Db16070 in TIA Portal e leggere l'offset assoluto
            # (colonna "Offset") di AppSpessoreMedio o aProfiloSpessore[0],
            # poi aggiornare DB_OFFSET_CORRECTION nel file .pyw.
            pos_ctr = sc.get('I_ParametriCntrolloSpessore.PosizioneCentroVentosa', None)
            rng_ctr = sc.get('I_ParametriCntrolloSpessore.RangeControllo', None)
            sp_rif  = sc.get('I_ParametriCntrolloSpessore.SpessoreDiscoRiferimento', None)
            sp_max  = sc.get('I_ParametriCntrolloSpessore.SpessoreMassimo', None)
            udt_ok  = (pos_ctr is not None
                       and abs(pos_ctr - 1930.0) < 1.0
                       and rng_ctr is not None
                       and abs(rng_ctr - 80.0) < 1.0)
            self._plog("  ── UDT params (attesi: Pos=1930 Range=80 Rif=2.98 SgMax=1.0) ──\n")
            self._plog(
                f"  PosizioneCentroVentosa={pos_ctr:.2f}  "
                f"RangeControllo={rng_ctr:.2f}  "
                f"SpessRif={sp_rif:.3f}  "
                f"SpessMax={sp_max:.3f}\n",
                "ok" if udt_ok else "err")
            if not udt_ok:
                self._plog(
                    "  ✗ Offset map SHIFTATA — UDT non corrisponde al file .db!\n"
                    "  → Aprire Db16070 in TIA Portal, colonna 'Offset'\n"
                    "  → Leggere offset di 'AppSpessoreMedio' e comunicarlo.\n"
                    "  → Poi impostare DB_OFFSET_CORRECTION nel file .pyw.\n",
                    "err")
            else:
                self._plog("  ✓ UDT corretta — offset map valida almeno fino a byte 36\n","ok")
    ...
    def _tab_autoexp(self, P):
        ...
        left=ttk.Frame(pane,width=390); pane.add(left,weight=0); left.pack_propagate(False)
        ...
        self._pv_poll=tk.StringVar(value="100")
        ...
        self._pv_trig=tk.StringVar(value="Falling edge Pinza.InZonaControllo")
        ttk.Combobox(r2,textvariable=self._pv_trig,width=32,state="readonly",
        ...
        tk.Label(lf2,text="Percorso (da Impostazioni):",bg=DARK_BG,fg=MUTED_CLR,
                 font=("Consolas",8)).pack(anchor="w")
        self._pv_ae_sql=tk.StringVar(value="—")
        tk.Label(lf2,textvariable=self._pv_ae_sql,bg=DARK_BG,fg=ACCENT,
                 font=("Consolas",9),wraplength=350,anchor="w",justify="left"
        ...
        for var,txt in [(self._pv_ae_viewer,"Carica automaticamente nel Viewer"),
            ...
            tk.Checkbutton(lf3,text=txt,variable=var,bg=DARK_BG,fg=TEXT_CLR,
                selectcolor="#1f6feb",activebackground=DARK_BG,font=("Consolas",9),
                anchor="w").pack(fill="x",pady=1)

        lf4=ttk.LabelFrame(left,text="  Monitoraggio  ",padding=6)
        lf4.pack(fill="x",padx=6,pady=4)
        rr=ttk.Frame(lf4); rr.pack(fill="x")
        ...
        tk.Label(lf4,textvariable=self._pv_ae_st,bg=DARK_BG,fg=MUTED_CLR,
                 font=("Consolas",10,"bold")).pack(anchor="w",pady=4)
        self._pv_ae_cnt=tk.StringVar(value="✓ 0  ✗ 0  ⚙ 0")
        tk.Label(lf4,textvariable=self._pv_ae_cnt,bg=DARK_BG,fg=AUTOEXP_CLR,
                 font=("Consolas",11,"bold")).pack(anchor="w")
        self._pv_ae_last=tk.StringVar(value="")
        tk.Label(lf4,textvariable=self._pv_ae_last,bg=DARK_BG,fg=MUTED_CLR,
                 font=("Consolas",8)).pack(anchor="w")
        self._pv_ae_rows=tk.StringVar(value="DB: — righe")
        tk.Label(lf4,textvariable=self._pv_ae_rows,bg=DARK_BG,fg=MUTED_CLR,
        ...
        self._alog("=== Auto-Export ===\n","info")
        self._alog("Connettiti al PLC (tab PLC Reader), poi premi Avvia.\n\n")
        self._upd_ae_sql()
    ...
    def _ae_start(self):
        if not self._plc_client:
            messagebox.showwarning("PLC non connesso","Connettiti prima.")
            self._nb.select(2); return
        ...
        self._ae_client=self._plc_client; self._ae_running=True; self._ae_prev=None
        self._ae_cok=self._ae_cnok=self._ae_ctar=0
        ...
        self._alog(f"▶ poll={self._pv_poll.get()}ms  trig={self._pv_trig.get()}\n","info")
        self._ae_poll()
    ...
    def _ae_poll(self):
        ...
        try:
            db=int(self._pv_db.get())
            raw=self._ae_client.read_db_raw(db, self._db_size,
                                             chunk=900,
                                             actual_size=self._actual_db_size)
            dec=decode_db(raw,self._omap); sc=dec['scalars']
            trig=self._pv_trig.get()
            if trig.startswith("Variazione"):
                sent=round(float(sc.get('AppSpessoreMedio',0)),6)
            elif trig.startswith("O_Tarat"):
                sent=bool(sc.get('O_TaraturaCompletata',False))
            else:
                sent=bool(sc.get('Pinza.InZonaControllo',False))
            triggered=False
            if self._ae_prev is None:
                self._ae_prev=sent
            else:
                if trig.startswith("Falling"):
                    triggered=(self._ae_prev and not sent)
                elif trig.startswith("O_Tarat"):
                    triggered=(not self._ae_prev and sent)
                else:
                    triggered=(sent!=self._ae_prev)
                self._ae_prev=sent
            if triggered: self._ae_trigger(dec,db)
        except Exception as e:
            self._alog(f"✗ polling: {e}\n","err"); self._pv_ae_st.set("● Errore")
        if self._ae_running:
            try: ms=max(20,int(self._pv_poll.get()))
            except: ms=100
            self._ae_timer=self.after(ms,self._ae_poll)

    def _ae_trigger(self, dec, db):
        sc=dec['scalars']; ts=datetime.datetime.now()
        is_tar=(bool(sc.get('O_TaraturaAttiva',False)) or
                self._pv_trig.get().startswith("O_Tarat"))
        ...
        self._alog(
            ...
            f"Sp={sc.get('AppSpessoreMedio',0):.3f}  "
            f"ΔMax={sc.get('AppDeltaMax',0):.3f}  "
            f"n={int(sc.get('AppNcelleProfilo',0)):>3d}  "
            f"fuori={nf:>3d}  #{rid}\n",tag)
        self._pv_ae_cnt.set(f"✓ {self._ae_cok}  ✗ {self._ae_cnok}  ⚙ {self._ae_ctar}")
        self._pv_ae_last.set(f"{icon} {ts.strftime('%H:%M:%S')}  #{rid}")
        ...
        if self._pv_ae_viewer.get():
            try:
                ...
                self._load_data(parse_db_text(txt,f"#{rid}"))
            except: pass
    ...
    def _tab_settings(self, P):
        ...
        notes.insert("end",f"""
{FB_TARGET}  |  DB: {self._db_size} byte ({self._db_size/1024:.1f} KB)
Array: {ARRAY_SIZE} celle [0..200]  |  Risoluzione: 0.80 mm/cella @ RangeControllo=80mm

CONFIGURAZIONE ATTUALE:
  DB istanza: Db16070_ControlloSpessore
  FB istanza: {FB_TARGET}
  S7_Optimized_Access := 'FALSE'  ✓  (lettura Snap7 con offset assoluti abilitata)
  DB number default: 16070

FISICA DEL SISTEMA:
  Laser posizionato SOTTO il piano di trasferimento.
  Quota letta DIMINUISCE all'aumentare dello spessore del disco.
  aBaseline[i] = laser_cal + SpessDiscoRif  = QUOTA SUPPORTO (invariante)
  aProfiloSpessore[i] = aBaseline - laser   = spessore reale disco
  aProfiloDelta[i]    = aProfiloSpessore - SpessDiscoRif = eccesso vs campione

NOK v1.1:
  Il confronto avviene su aProfiloDelta (eccesso), NON su aProfiloSpessore (assoluto).
  SpessoreMassimo = max delta tollerato [mm]  (NON soglia assoluta!)
  Disco ok (2.98mm):   aProfiloDelta ≈ 0.00 → OK ✓
  Doppio disco (~6mm): aProfiloDelta ≈ 3.00 > SpessoreMassimo(1.0) → NOK ✓

AppNcelleFuoriSoglia @ offset 4992 (v1.1):
  In v1.0 era VAR_TEMP → si azzerava ogni ciclo OB.
  In v1.1 è in VAR → mantiene il valore tra cicli → HMI legge correttamente.

Parametri reali DB16070:
  PosizioneCentroVentosa = 1930.0 mm
  RangeControllo = 80.0 mm  →  finestra ±80mm = 160mm totali
  SpessoreDiscoRiferimento = 2.98 mm
  SpessoreMassimo = 1.0 mm  (eccesso max tollerato)
  OldDirLavoro = 2  (solo negative)
""")
