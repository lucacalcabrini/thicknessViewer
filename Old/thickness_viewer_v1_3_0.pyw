...
class ThicknessApp(tk.Tk):
    ...
    def _tab_autoexp(self, P):
        ...
        ttk.Label(r1,text="ms  (poll veloce IdLettura — lettura completa solo su variazione)",
                  style="Muted.TLabel").pack(side="left")

        # Info trigger
        info_trig=tk.Label(lf1,
            text="Trigger: IdLettura DInt @ offset 4996\n"
                 "Il PLC incrementa IdLettura a ogni fine passata (Fn.Q).\n"
                 "Python legge solo 4 byte per ciclo, DB completo solo su variazione.",
            bg=DARK_BG,fg=MUTED_CLR,font=("Consolas",8),justify="left",anchor="w")
        info_trig.pack(fill="x",pady=(2,0))
    ...
    def _ae_start(self):
        ...
        self._alog(f"▶ poll={self._pv_poll.get()}ms  trigger=IdLettura@4996\n","info")
        self._ae_poll()
    ...
    def _ae_poll(self):
        ...
        try:
            ...
            # ── 1. Leggi solo IdLettura (4 byte) ─────────────────────────────
            counter = self._ae_client.read_counter(db, offset=4996)

            if counter is None:
                self._alog("⚠ Lettura IdLettura fallita\n","warn")
            elif self._ae_prev is None:
                # Prima lettura: memorizza il valore corrente e aspetta variazione
                self._ae_prev = counter
                self._alog(f"✓ IdLettura iniziale: {counter}\n","info")
            elif counter != self._ae_prev:
                # ── 2. Variazione rilevata → leggi i dati completi ───────────
                self._ae_prev = counter
                raw = self._ae_client.read_db_raw(
                    db, self._db_size, actual_size=self._actual_db_size)
                dec = decode_db(raw, self._omap)
                self._ae_trigger(dec, db, counter)
    ...
    def _ae_trigger(self, dec, db, counter=0):
